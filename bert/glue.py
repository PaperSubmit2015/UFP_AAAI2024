...
def main():
    ...
    ### added code by ice; to traverse all  layers and record names
    args.epochs = starting_epoch + args.num_train_epochs

    args.arch= args.task_name
    args.loglevel = 1
    args.start_epoch=starting_epoch
    args.qasr = "self"
    args.qwsr = "self"
    args.qgsr = "self"
    ...
    #getLayerNames(model) 
    #model.apply(traverseLayer)
    #model.apply(modelConfigInit)
    #model.apply(init_weight_hp_copy)

 
    #setupModelQuantEnv(model, "bertmxfp8.json", qname = args.task_name)
    setupQuantEnv( "bertfp8.json", qname = args.task_name)
    configModel(model)
    

    args.logpath = QtArgs.logpath
    
    model.apply(add_hooks_for_eci_train)   # quant_activation
    resetTrainCurve("accuracy")
    resetTrainCurve("glue")
    resetTrainCurve('pearson')
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if step % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                optimizer.step()
                model.apply( backward_update_hp)     ##  ECI
                lr_scheduler.step()
                optimizer.zero_grad()
        ...
        if args.with_tracking:
            ...
            # record curve data and save to files
            print("[DEBUG]total_loss=%f"%(total_loss))
            appendTrainCurveData("loss",total_loss.item()/len(train_dataloader) )
            ...
            appendTrainCurveData(metrics , eval_metric[metrics])
            recordTrainCurve(metrics )
